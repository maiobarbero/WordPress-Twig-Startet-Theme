...
const projectURL = `http://${info.name}.local/`
const paths = {
	styles: {
...
export const styles = () => {
	...
	return gulp
		...
		.pipe(replace('/*!', '/*'))
		.pipe(
			replace('{{theme.description}}', info.description + ' - ' + info.version)
		)
		.pipe(replace('{{theme.version}}', info.version))
		.pipe(replace('{{theme.keywords}}', info.keywords))
